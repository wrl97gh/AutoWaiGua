...
def merge_detection(model, image):
    ...
    top = round(height * 120 / 768)     # 120px at 768p, scaled to the actual resolution
    cropped = image[top:height//2, width//4:3*width//4]
    ...
    if len(boxes) == 4:      # Only run further inferences if arrows have been correctly detected
        ...
        # The rotated pass only refines horizontal arrows, so its inference
        # can be skipped entirely when the upright pass found none.
        if any(c in ('left', 'right') for c in classes):
