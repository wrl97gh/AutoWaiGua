...
from src.common import config, controls, platform, utils
from datetime import datetime


class Listener(Configurable):
    DEFAULT_CONFIG = {
        'Start/stop': 'f8' if platform.IS_MACOS else 'insert',
        'Reload routine': 'f6',
        'Record position': 'f7'
    }
    ...
    def __init__(self):
        ...
        if platform.IS_MACOS and self.config.get('Start/stop') == 'insert':
            self.config['Start/stop'] = 'f8'
            self.save_config()
    ...
    def _main(self):
        ...
        while True:
            if self.enabled:
                if controls.is_pressed(self.config['Start/stop']):
                    Listener.toggle_enabled()
                elif controls.is_pressed(self.config['Reload routine']):
    ...
    def restricted_pressed(self, action):
        ...
        if controls.is_pressed(self.config[action]):
    ...
    def toggle_enabled():
        ...
        if config.enabled:
            platform.beep(784, 333)     # G5
        else:
            platform.beep(523, 333)     # C5
        time.sleep(0.267)

    @staticmethod
    def reload_routine():
        if not config.routine.path:
            config.routine.load()
            time.sleep(0.6)
            return
        ...
        platform.beep(523, 200)     # C5
        platform.beep(659, 200)     # E5
        platform.beep(784, 200)     # G5
