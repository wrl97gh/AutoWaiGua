...
class Capture:
    ...
    def _main(self):
        ...
        while True:
            ...
            match_profile = self._select_match_profile(self.frame)
            mm_tl_template = match_profile['tl_template']
            mm_br_template = match_profile['br_template']
            player_template = match_profile['player_template']
            player_height, player_width = player_template.shape
            minimap_top_border = match_profile['top_border']
            minimap_bottom_border = match_profile['bottom_border']
            self.minimap_template_scale = match_profile['template_scale']

            tl, _ = self._single_match(self.frame, mm_tl_template)
            _, br = self._single_match(self.frame, mm_br_template)
            mm_tl = (
                tl[0] + minimap_bottom_border,
                tl[1] + minimap_top_border
            )
            mm_br = (
                max(mm_tl[0] + player_width, br[0] - minimap_bottom_border),
                max(mm_tl[1] + player_height, br[1] - minimap_bottom_border)
            )
