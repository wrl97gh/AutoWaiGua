...
from src.common import controls, utils
...
class KeyBindings(LabelFrame):
    DEFAULT_SCROLL_HEIGHT = 240
    LONG_SCROLL_HEIGHT = 650

    def __init__(self, parent, label, target, **kwargs):
        self.scroll_height = kwargs.pop('scroll_height', None)
    ...
    def create_edit_ui(self):
        ...
        if height is None:
            height = KeyBindings.LONG_SCROLL_HEIGHT if len(self.target.config) > 27 else KeyBindings.DEFAULT_SCROLL_HEIGHT
        self.container = Frame(self, width=354, height=height)
        ...
        self.canvas_window = self.canvas.create_window((0, 0), window=self.contents, anchor=tk.NW)
    ...
    def create_entry(self, action, key):
        ...
        def on_key_press(event):
            k = controls.tk_event_to_key_name(event)
